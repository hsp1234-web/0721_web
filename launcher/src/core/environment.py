...
class EnvironmentManager:
    ...
    def __init__(self, project_path: Path, min_disk_space_mb: int = 100, max_memory_usage_percent: int = 95):
    ...
    def check_disk_space(self) -> bool:
        ...
        if free_space_gb * 1024 < self.min_disk_space_mb:
            console.print(f"[bold red]錯誤: 磁碟空間不足！至少需要 {self.min_disk_space_mb} MB。[/bold red]")
            return False
        return True
    ...
    def check_memory(self) -> bool:
        ...
        if memory_percent > self.max_memory_usage_percent:
            console.print(f"[bold red]錯誤: 記憶體使用率過高！目前為 {memory_percent}%。[/bold red]")
            return False
        return True

    def setup_project_directory(self, force_refresh: bool, repo_url: str, branch: str):
        """設定專案目錄，如果不存在則從 Git clone。"""
        if self.project_path.exists() and force_refresh:
            console.log(f"偵測到 '強制刷新' 選項，正在刪除舊的專案目錄: {self.project_path}")
            shutil.rmtree(self.project_path)

        if not self.project_path.exists():
            console.log(f"專案目錄不存在，正在從 {repo_url} (分支: {branch}) 下載...")
            try:
