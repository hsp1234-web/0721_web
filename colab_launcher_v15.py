...
# ║                                                                      ║
# ║   設計哲學：                                                         ║
# ║       本版本是所有迭代的結晶，既保持了 v10.1 的簡潔與分離，         ║
# ║       又整合了所有從錯誤中學到的穩健性原則。                         ║
# ║                                                                      ║
# ║   v15 更新 (Jules 修正):                                             ║
# ║       - 優雅的超時：引入帶進度條的 90 秒啟動超時，不再無限等待。     ║
# ║       - 根除埠衝突：採用 `psutil` 進行精準的埠清理，解決當機問題。     ║
# ║       - 安裝進度條：移除 `-q` 參數，讓使用者可以看到安裝過程。         ║
# ║       - 分離式執行：堅持使用 `subprocess` 執行核心，確保環境純淨。   ║
# ║                                                                      ║
# ╚══════════════════════════════════════════════════════════════════╝

#@title 💎 鳳凰之心指揮中心 v15 (啟示錄版) { vertical-output: true, display-mode: "form" }
#@markdown ---
