...
if [ ! -f "$PROJECT_ROOT/integrated_platform/pyproject.toml" ]; then
    echo "💥 [致命錯誤] 檢查失敗：在專案根目錄 '${PROJECT_ROOT}/integrated_platform' 中找不到 'pyproject.toml' 檔案！"
...
(cd "$PROJECT_ROOT/integrated_platform" && poetry lock)
(cd "$PROJECT_ROOT/integrated_platform" && poetry check)
echo "    ✅ Poetry 確認設計圖有效。"
...
echo "🚀 [4/5] 正在更新 lock 檔案並安裝專案所有依賴..."
(cd "$PROJECT_ROOT/integrated_platform" && poetry lock)
(cd "$PROJECT_ROOT/integrated_platform" && poetry install --no-root)
echo "✅ 所有專案依賴已安裝完成。"
