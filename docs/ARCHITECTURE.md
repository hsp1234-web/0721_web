...
## 五、V3 核心架構：Colab 原生 TUI 與統一日誌系統

V3 架構是對先前方案的重大革新，其核心目標是**簡化與統一**。我們拋棄了所有 Web 前端技術和複雜的資料庫驅動方案，回歸到一個由單一 Python 腳本主導，在 Colab/Jupyter 環境中直接渲染文字使用者介面 (TUI) 的高效模型。

### 核心變更：

*   **單一腳本主導**：`launch.py` 現在是唯一的入口點，完全負責任務執行和 TUI 的渲染。
*   **Colab 原生 TUI**：在 Colab/Jupyter 輸出儲存格中實現了一個高效、穩定的雙區塊儀表板。
*   **統一日誌系統**：所有事件（包括高頻效能數據）現在都被記錄到一個統一的 `logs.sqlite` 資料庫中，作為系統的絕對真相來源。
*   **自動化報告生成**：任務結束時，系統會自動從 `logs.sqlite` 中讀取數據，生成三份詳細的 Markdown 報告（綜合、效能、日誌），實現了完整的任務歸檔。

### 雙區塊智能監控策略

這是 V3 TUI 的核心實現，由後端 Python 腳本直接渲染。

*   **A. 下半部：恆定「即時狀態」行 (高頻更新)**
    *   **技術**：使用獨立的背景執行緒 (thread)，以高頻率（如每 0.1 秒）`print("\r" + "新狀態...", end="", flush=True)`。\r 控制碼實現了完全無閃爍的原地刷新。
    *   **內容**：`HH:MM:SS | CPU: XX.X% | RAM: YY.Y% | [核心進度標籤]`

*   **B. 上半部：固定窗口「近況彙報」區 (低頻更新)**
    *   **技術**：主執行緒維護一個日誌緩衝區 (`collections.deque`)。只有當新的關鍵日誌產生時，才觸發一次 `clear_output(wait=True)`，並重繪整個畫面（先印上半部，再印下半部）。
    *   **體驗**：使用者看到的是穩定、低頻率跳動的「事件摘要」，而非滾動的日誌瀑布。

### 後端記錄與歸檔機制

*   **絕對完整性**：無論前端顯示什麼，系統中所有類型的日誌（包括 DEBUG 和高頻效能數據）都將以毫秒級時間戳、Asia/Taipei 時區，全速寫入後端的 `logs.sqlite` 資料庫。這是系統的絕對真相來源。
*   **自動化歸檔**：任務結束時，系統會自動從 `logs.sqlite` 中讀取所有紀錄，並生成三份 `.md` 格式的最終報告，供使用者下載和分析。
