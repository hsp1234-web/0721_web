...
└── 📄 requirements-dev.txt        # 【推薦】開發與測試環境的依賴鎖定檔案。
```
...
## 五、V12+ 架構演進：日誌篩選與依賴強化

在 v12 的資料庫驅動架構基礎上，我們進一步引入了更精細的控制和更強大的安全保障。

### **1. 後端日誌過濾 (Backend Log Filtering)**

- **動機**: 前端儀表板的日誌區域空間有限，使用者在除錯時，往往只關心特定等級的日誌（如 `ERROR`, `CRITICAL`）。在前端進行過濾會導致不必要的數據傳輸，且無法減少後端資料庫的查詢負擔。
- **架構決策**: 我們選擇在**後端 API (`get_status_api`)** 中實現日誌過濾邏輯。
    1.  Colab UI 提供一組日誌等級的勾選框。
    2.  這些設定被寫入 `config.json`，作為唯一的真相來源。
    3.  後端 `get_status_api` 在每次被呼叫時，會讀取 `config.json` 中的設定。
    4.  API 在查詢 SQLite 資料庫時，動態地建構 `WHERE level IN (...)` 的 SQL 語句。
- **優勢**:
    - **高效**: 過濾在資料庫層級完成，只傳輸使用者需要的數據，最大化地減少了網路負載。
    - **解耦**: 前端 JavaScript 無需任何修改，它只負責忠實地呈現 API 回傳的任何數據。
    - **中心化**: 所有過濾邏輯都集中在後端 API，易於維護和修改。

### **2. 依賴鎖定與安全審計 (Dependency Locking & Security Auditing)**

- **動機**: 在多人協作和 CI/CD 環境中，`requirements.txt` 中未鎖定版本的依賴（如 `pandas`）是「定時炸彈」，可能導致今天成功的建置明天就失敗。同時，開源套件的漏洞也對專案安全構成威脅。
- **架構決策**: 我們將依賴管理和安全審計整合進了核心測試流程 `smart_e2e_test.py` 中。
    1.  **依賴鎖定 (`pip-tools`)**: 我們引入了 `pip-tools`。開發者在 `requirements.in` 檔案中聲明直接依賴，然後使用 `pip-compile` 生成完全鎖定、包含所有間接依賴的 `requirements.txt` 檔案。這確保了環境的**可重現性 (Reproducibility)**。
    2.  **安全審計 (`pip-audit`)**: 在 `smart_e2e_test.py` 的測試流程中，我們新增了一個步驟。在為每個 App 安裝完所有依賴後，會立即呼叫 `pip-audit` 來掃描當前虛擬環境中的所有套件是否存在已知的安全漏洞 (CVEs)。
- **優勢**:
    - **穩定**: `pip-tools` 杜絕了因依賴版本變動導致的意外失敗。
    - **安全**: `pip-audit` 的整合將安全檢查「左移」(Shift Left)，在開發和測試階段就能發現並阻止有漏洞的依賴被引入，而非等到上線後才發現。
    - **自動化**: 這兩項檢查都已完全整合到自動化測試腳本中，無需人工干預。

---

## 六、V4 核心架構：預設快速驗證與參數化完整執行
