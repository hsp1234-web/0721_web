...
# ║                                                                      ║
# ║   設計哲學：                                                         ║
# ║       本版本回歸 v10.1 的簡潔設計，將啟動器與核心邏輯分離。         ║
# ║       啟動器 (Launcher) 專注於環境準備，核心邏輯 (Core) 則由        ║
# ║       一個獨立的腳本 `scripts/colab_run.py` 處理。                   ║
# ║                                                                      ║
# ║   v13 更新 (Jules 修正):                                             ║
# ║       - 根除匯入錯誤：採用 `subprocess` 執行核心邏輯，而不是 `import`，║
# ║         這徹底解決了所有 `ModuleNotFoundError` 問題。                ║
# ║       - 依賴前置：確保在執行任何程式碼前，所有依賴都已被安裝。       ║
# ║       - 參數傳遞：透過環境變數，將 Colab 介面的參數安全地傳遞給     ║
# ║         核心邏輯腳本。                                               ║
# ║                                                                      ║
# ╚══════════════════════════════════════════════════════════════════╝

#@title 💎 鳳凰之心指揮中心 v13 (鳳凰版) { vertical-output: true, display-mode: "form" }
#@markdown ---
