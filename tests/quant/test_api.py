...
def test_root_path():
    ...
    response = client.get("/")
    assert response.status_code == 200
    assert response.json() == {"message": "歡迎來到量化金融服務 API"}
...
def test_perform_backtest_api(mocker):
    ...
    mocker.patch(
        "quant.logic.analysis.run_simple_backtest",
        return_value=mock_success_result
    )

    # 模擬 database.db_manager.save_backtest_result 函式，因為我們不想真的寫入資料庫
    mock_db_save = mocker.patch("quant.logic.database.db_manager.save_backtest_result")
    ...
    response = client.post("/v1/backtest", json=request_payload)
