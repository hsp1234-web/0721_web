...
class TestUbuntuBackend(unittest.TestCase):

    def setUp(self):
        self.test_project_path = PROJECT_ROOT / "test_ubuntu_project"
    ...
    def test_backend_flow_and_safe_installer(self):
        # 1. 準備環境
        env = os.environ.copy()
        ...
        # 2. 啟動後端服務
        launch_log = self.test_project_path / "launch.log"
        api_log = self.test_project_path / "api_server.log"

        with open(launch_log, "w") as f_launch, open(api_log, "w") as f_api:
            launch_process = subprocess.Popen(
                [sys.executable, str(PROJECT_ROOT / "launch.py")],
                env=env, stdout=f_launch, stderr=subprocess.STDOUT, cwd=self.test_project_path
            )
            api_process = subprocess.Popen(
                [sys.executable, str(PROJECT_ROOT / "api_server.py")],
                env=env, stdout=f_api, stderr=subprocess.STDOUT, cwd=PROJECT_ROOT
            )

            # 3. 健康檢查
            self.assertTrue(self._wait_for_service(8081), "後端服務在指定時間內未啟動")

            # 4. 驗證 API
            response = httpx.get("http://localhost:8081/api/status")
